...
import RegisterVM hiding (Proc (..), Op (..), boolToInt, intToBool)
...
compileProgram :: [Definition] -> Compile RegisterVM.Program
compileProgram defs = do
    ...
    case mainProc of
        ...
        Just proc -> do 
            when ((RegisterVM.nArgs proc) /= 0) $ compileError "main must take no arguments"
            ps <- getProcs
            pure $ RegisterVM.Program {mainProc=proc, allProcs=ps}
...
toVMProc :: ProcIR Label VarId -> Compile (RegisterVM.Proc)
toVMProc (ProcIR funId params vars code) = do
    ...
    pure $ RegisterVM.Proc nParams nVars code'


toVMOp :: OpIR Int VarIx -> RegisterVM.Op
toVMOp (Load ix)   = RegisterVM.Load ix
toVMOp (Store ix)  = RegisterVM.Store ix
toVMOp (Nop)       = RegisterVM.Nop
toVMOp (Push val)  = RegisterVM.Push val
toVMOp (Pop)       = RegisterVM.Pop
toVMOp (Dup)       = RegisterVM.Dup
toVMOp (Add )      = RegisterVM.Add 
toVMOp (Mul )      = RegisterVM.Mul 
toVMOp (Sub )      = RegisterVM.Sub 
toVMOp (Incr )     = RegisterVM.Incr 
toVMOp (Decr)      = RegisterVM.Decr
toVMOp (Equal       ) = RegisterVM.Equal
toVMOp (Less        ) = RegisterVM.Less
toVMOp (Greater     ) = RegisterVM.Greater
toVMOp (LessEqual   ) = RegisterVM.LessEqual
toVMOp (GreaterEqual) = RegisterVM.GreaterEqual
toVMOp (Not)       = RegisterVM.Not
toVMOp (Jmp   off) = RegisterVM.Jmp   off 
toVMOp (JmpIf off) = RegisterVM.JmpIf off
toVMOp (Call id n) = RegisterVM.Call id n
toVMOp (Ret)       = RegisterVM.Ret
